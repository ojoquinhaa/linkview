...
// .wrangler/tmp/bundle-kwW6xX/strip-cf-connecting-ip-header.js
...
// ../../node_modules/.pnpm/wrangler@3.114.17_@cloudflare+workers-types@4.20260615.1/node_modules/wrangler/_virtual_unenv_global_polyfill-@cloudflare-unenv-preset-node-console
...
// ../../node_modules/.pnpm/wrangler@3.114.17_@cloudflare+workers-types@4.20260615.1/node_modules/wrangler/_virtual_unenv_global_polyfill-@cloudflare-unenv-preset-node-process
...
function isValidCpf(input) {
  ...
  if (/^(\d)\1{10}$/.test(cpf))
    return false;
  const digit = /* @__PURE__ */ __name((sliceLength) => {
    let sum = 0;
...
__name(isValidDocument, "isValidDocument");
...
var PERSON_TYPES = ["pf", "pj"];

// ../../packages/shared/src/url.ts
var ALLOWED_PROTOCOLS = /* @__PURE__ */ new Set(["http:", "https:", "tel:", "mailto:"]);
...
function validateDestinationUrl(input) {
  let url;
  try {
    url = new URL(input);
  } catch {
...
__name(validateDestinationUrl, "validateDestinationUrl");
function isValidDestinationUrl(input) {
  return validateDestinationUrl(input) === null;
}
__name(isValidDestinationUrl, "isValidDestinationUrl");
...
var destinationUrl = external_exports.string().url().refine(isValidDestinationUrl, { message: "URL de destino n\xE3o permitida" });
var slugField = external_exports.string().min(SLUG_MIN_LENGTH).max(SLUG_MAX_LENGTH).regex(/^[a-z0-9][a-z0-9_-]*[a-z0-9]$/, { message: "Slug inv\xE1lido" });
var utmSchema = external_exports.object({
  utmSource: external_exports.string().max(255).optional(),
  utmMedium: external_exports.string().max(255).optional(),
  utmCampaign: external_exports.string().max(255).optional(),
  utmTerm: external_exports.string().max(255).optional(),
  utmContent: external_exports.string().max(255).optional()
});
var createLinkSchema = external_exports.object({
  destinationUrl,
  slug: slugField.optional(),
  title: external_exports.string().max(255).optional(),
  description: external_exports.string().max(1e3).optional(),
  campaignId: external_exports.string().uuid().optional(),
  domainId: external_exports.string().uuid().optional(),
  expiresAt: external_exports.coerce.date().optional(),
  password: external_exports.string().min(4).max(128).optional(),
  tags: external_exports.array(external_exports.string().max(64)).max(20).optional()
}).merge(utmSchema);
var updateLinkSchema = createLinkSchema.partial().extend({ isActive: external_exports.boolean().optional() });
var registerSchema = external_exports.object({
  name: external_exports.string().trim().min(2, "Informe seu nome").max(120),
  email: external_exports.string().trim().toLowerCase().email("E-mail inv\xE1lido").max(255),
  password: external_exports.string().min(8, "A senha precisa de ao menos 8 caracteres").max(128),
  personType: external_exports.enum(PERSON_TYPES),
  document: external_exports.string().min(1, "Informe o documento"),
  phone: external_exports.string().refine(isValidPhone, "Telefone inv\xE1lido"),
  zip: external_exports.string().refine((v) => v.replace(/\D/g, "").length === 8, "CEP inv\xE1lido"),
  street: external_exports.string().trim().min(1, "Informe o logradouro").max(200),
  number: external_exports.string().trim().min(1, "Informe o n\xFAmero").max(20),
  complement: external_exports.string().trim().max(120).optional().default(""),
  district: external_exports.string().trim().min(1, "Informe o bairro").max(120),
  city: external_exports.string().trim().min(1, "Informe a cidade").max(120),
  state: external_exports.string().trim().toUpperCase().regex(/^[A-Z]{2}$/, "UF inv\xE1lida"),
  acceptTerms: external_exports.literal(true, {
    message: "\xC9 necess\xE1rio aceitar os Termos de Uso"
  }),
  acceptPrivacy: external_exports.literal(true, {
    message: "\xC9 necess\xE1rio aceitar a Pol\xEDtica de Privacidade"
  }),
  marketingOptIn: external_exports.boolean().default(false)
}).refine((data) => isValidDocument(data.document, data.personType), {
  message: "Documento inv\xE1lido",
  path: ["document"]
});
...
var ogSchema = external_exports.object({
  ogTitle: external_exports.string().max(120).optional(),
  ogDescription: external_exports.string().max(300).optional(),
  ogImageUrl: external_exports.string().url().max(2048).optional()
});
var LOGO_POSITIONS = ["top", "center", "bottom"];
var SPLASH_BG_TYPES = ["color", "image"];
var hexColor = external_exports.string().regex(/^#[0-9a-fA-F]{6}$/, { message: "Use uma cor hex (#RRGGBB)." });
var splashConfigSchema = external_exports.object({
  /** Logo URL, or null to use the urlsimples wordmark. */
  logoUrl: external_exports.string().nullable(),
  bgType: external_exports.enum(SPLASH_BG_TYPES),
  bgColor: external_exports.string(),
  bgImageUrl: external_exports.string().nullable(),
  blur: external_exports.number(),
  logoPosition: external_exports.enum(LOGO_POSITIONS),
  accentColor: external_exports.string(),
  textColor: external_exports.string(),
  countdownSeconds: external_exports.number(),
  /** When true, render the "Criado com urlsimples" footer mark. */
  showBranding: external_exports.boolean()
});
var pageLayoutSchema = external_exports.object({
  name: external_exports.string().trim().min(1, "D\xEA um nome ao layout").max(80),
  logoUrl: external_exports.string().url().max(2048).nullable().optional(),
  bgType: external_exports.enum(SPLASH_BG_TYPES).default("color"),
  bgColor: hexColor.default("#0b0b0f"),
  bgImageUrl: external_exports.string().url().max(2048).nullable().optional(),
  blur: external_exports.number().int().min(0).max(40).default(0),
  logoPosition: external_exports.enum(LOGO_POSITIONS).default("center"),
  accentColor: hexColor.default("#6366f1"),
  textColor: hexColor.default("#ffffff"),
  countdownSeconds: external_exports.number().int().min(1).max(15).default(3),
  showBranding: external_exports.boolean().default(true)
});
var createChannelSchema = external_exports.object({
  name: external_exports.string().min(1, "D\xEA um nome ao canal").max(60),
  utmSource: external_exports.string().min(1).max(120),
  utmMedium: external_exports.string().max(120).optional(),
  utmCampaign: external_exports.string().max(120).optional()
});
var createCampaignSchema = external_exports.object({
  name: external_exports.string().min(1).max(255),
  slug: slugField.optional(),
  description: external_exports.string().max(1e3).optional(),
  startsAt: external_exports.coerce.date().optional(),
  endsAt: external_exports.coerce.date().optional()
});
var countryCode = external_exports.string().trim().toUpperCase().regex(/^[A-Z]{2}$/, { message: "C\xF3digo de pa\xEDs inv\xE1lido (use BR, US\u2026)" });
var linkSecuritySchema = external_exports.object({
  isActive: external_exports.boolean().optional(),
  expiresAt: external_exports.coerce.date().nullable().optional(),
  /** `null`/empty removes the password; a value sets a new one. */
  password: external_exports.string().min(4).max(128).nullable().optional(),
  maxClicks: external_exports.number().int().min(1).max(1e7).nullable().optional(),
  blockBots: external_exports.boolean().optional(),
  allowedCountries: external_exports.array(countryCode).max(50).optional(),
  blockedCountries: external_exports.array(countryCode).max(50).optional(),
  rateLimitPerMinute: external_exports.number().int().min(1).max(1e4).nullable().optional()
});
var kvLinkRecordSchema = external_exports.object({
  linkId: external_exports.string(),
  workspaceId: external_exports.string(),
  destinationUrl: external_exports.string().url(),
...
var clickIngestSchema = external_exports.object({
  ...
  bot: external_exports.boolean().optional(),
  source: external_exports.string().optional(),
  medium: external_exports.string().optional(),
  campaign: external_exports.string().optional()
});
...
// src/ua.ts
var BOT_RE = /bot|crawl|spider|slurp|bingpreview|facebookexternalhit|whatsapp|telegrambot|preview|monitor|curl|wget|python-requests|headless/i;
function parseUa(ua) {
  if (!ua)
  ...
  const device = bot ? "bot" : isTablet ? "tablet" : isMobile ? "mobile" : "desktop";
  let browser = "unknown";
  if (/edg/i.test(ua))
    browser = "Edge";
...
async function buildClick(req, link, env2) {
  ...
  return {
    ...
    qrCodeId: url.searchParams.get("qr") ?? void 0,
    occurredAt: (/* @__PURE__ */ new Date()).toISOString(),
    ipHash: ip ? await hashIp(ip, env2.IP_HASH_SALT) : void 0,
    userAgent: ua ?? void 0,
    referer: req.headers.get("referer") ?? void 0,
    country: cf.country ?? req.headers.get("cf-ipcountry") ?? void 0,
    region: cf.region,
    city: cf.city,
...
__name(buildClick, "buildClick");
async function sendClick(payload, env2) {
  if (!env2.CLICK_INGEST_URL)
    return;
  try {
    await fetch(env2.CLICK_INGEST_URL, {
      method: "POST",
      headers: {
        "content-type": "application/json",
        ...env2.CLICK_INGEST_TOKEN ? { authorization: `Bearer ${env2.CLICK_INGEST_TOKEN}` } : {}
      },
...
__name(sendClick, "sendClick");

// src/security.ts
function unlockCookieName(linkId) {
  return `us_pw_${linkId}`;
}
__name(unlockCookieName, "unlockCookieName");
var UNLOCK_TTL_MS = 12 * 60 * 60 * 1e3;
...
async function makeUnlockToken(linkId, secret) {
  const exp = Date.now() + UNLOCK_TTL_MS;
  const sig = await hmacHex(secret, `${linkId}.${exp}`);
  return `${exp}.${sig}`;
}
__name(makeUnlockToken, "makeUnlockToken");
async function verifyUnlockToken(token, linkId, secret) {
...
__name(verifyUnlockToken, "verifyUnlockToken");
function countryBlocked(link, country) {
  const c = (country ?? "").toUpperCase();
  const allow = (link.allowedCountries ?? []).map((x) => x.toUpperCase());
  const block = (link.blockedCountries ?? []).map((x) => x.toUpperCase());
  if (allow.length > 0 && (!c || !allow.includes(c)))
    return true;
  if (c && block.includes(c))
    return true;
  return false;
}
__name(countryBlocked, "countryBlocked");
async function rateLimited(env2, linkId, ipHash, limitPerMinute) {
  const bucket = Math.floor(Date.now() / 6e4);
  const key = `rl:${linkId}:${ipHash}:${bucket}`;
  const current = Number(await env2.LINKS_KV.get(key) ?? "0");
  if (current >= limitPerMinute)
    return true;
  await env2.LINKS_KV.put(key, String(current + 1), { expirationTtl: 120 });
  return false;
}
__name(rateLimited, "rateLimited");
async function verifyPasswordRemote(env2, hostname, slug, password) {
  if (!env2.UNLOCK_VERIFY_URL)
    return false;
  try {
    const res = await fetch(env2.UNLOCK_VERIFY_URL, {
      method: "POST",
      headers: {
        "content-type": "application/json",
        ...env2.CLICK_INGEST_TOKEN ? { authorization: `Bearer ${env2.CLICK_INGEST_TOKEN}` } : {}
...
__name(verifyPasswordRemote, "verifyPasswordRemote");
function unlockPage(error3 = false) {
  const msg = error3 ? `<p class="err">Senha incorreta. Tente de novo.</p>` : "";
...
// src/index.ts
var app = new Hono2();
function errorPage(title2, message, status) {
  const html = `<!doctype html><html lang="pt-BR"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${title2}</title><style>body{font-family:system-ui,sans-serif;background:#0b0b0f;color:#e5e5e5;display:flex;min-height:100vh;align-items:center;justify-content:center;margin:0}main{text-align:center;padding:2rem}h1{font-size:3rem;margin:0}p{color:#a1a1aa}</style></head><body><main><h1>${status}</h1><p>${message}</p></main></body></html>`;
  return new Response(html, {
    status,
...
function clientCountry(req) {
  const cf = req.cf;
  return cf?.country ?? req.headers.get("cf-ipcountry") ?? void 0;
}
__name(clientCountry, "clientCountry");
function clientIp(req) {
  return req.headers.get("cf-connecting-ip") ?? req.headers.get("x-forwarded-for")?.split(",")[0]?.trim() ?? "";
}
__name(clientIp, "clientIp");
function recordClick(c, link) {
  c.executionCtx.waitUntil(
    buildClick(c.req.raw, link, c.env).then(
      (payload) => sendClick(payload, c.env)
    )
  );
}
__name(recordClick, "recordClick");
function doRedirect(c, link) {
  recordClick(c, link);
  return c.redirect(link.destinationUrl, 302);
}
__name(doRedirect, "doRedirect");
async function loadGatedLink(c) {
  const slug = c.req.param("slug");
  const hostname = new URL(c.req.url).hostname;
  const raw2 = await c.env.LINKS_KV.get(`link:${hostname}:${slug}`, "json");
  if (!raw2) {
    return { error: errorPage("N\xE3o encontrado", "Este link n\xE3o existe.", 404) };
  }
  const parsed = kvLinkRecordSchema.safeParse(raw2);
  if (!parsed.success) {
    return { error: errorPage("Erro", "Link inv\xE1lido.", 404) };
  }
  const link = parsed.data;
  if (!link.active) {
    return {
      error: errorPage("Indispon\xEDvel", "Este link foi desativado.", 403)
    };
  }
  ...
  if (link.blockBots && parseUa(c.req.header("user-agent")).bot) {
    return {
      error: errorPage("Indispon\xEDvel", "Acesso n\xE3o permitido.", 403)
    };
  }
  if (countryBlocked(link, clientCountry(c.req.raw))) {
    return {
      error: errorPage(
        "Indispon\xEDvel",
        "Este link n\xE3o est\xE1 dispon\xEDvel na sua regi\xE3o.",
        403
      )
    };
  }
  if (link.rateLimitPerMinute && link.rateLimitPerMinute > 0) {
    ...
    if (ip) {
      ...
      if (await rateLimited(c.env, link.linkId, ipHash, link.rateLimitPerMinute)) {
        return {
          error: errorPage(
            "Muitas tentativas",
            "Voc\xEA acessou este link muitas vezes. Aguarde um minuto.",
            429
          )
        };
      }
...
__name(loadGatedLink, "loadGatedLink");
async function isUnlocked(c, linkId) {
...
__name(isUnlocked, "isUnlocked");
app.get(
  "/",
  (c) => c.text("urlsimples redirect engine", 200, { "cache-control": "no-store" })
);
...
// .wrangler/tmp/bundle-kwW6xX/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-kwW6xX/middleware-loader.entry.ts
